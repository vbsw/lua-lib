...
function testCL()
	local cmdLine = clNew({"asdf", "--version"})
	...
	cmdLine = clNew({"--start", "asdf", "-s", "qwer"})
	...
	cmdLine = clNew({"asdf", "--start=123"})
	assertEq(cmdLine.n, 2)
	cmdLine.delimiter = clNewDelimiter("=", "")
