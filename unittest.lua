...
function testCL()
	local cmdLine = newCL({"asdf", "--version"})
	...
	cmdLine = newCL({"--start", "asdf", "-s", "qwer"})
	...
	cmdLine = newCL({"asdf", "--start=123"})
	assertEq(cmdLine.n, 2)
	cmdLine.delimiter = newCLDelimiter("=", "")
	asserType(cmdLine.delimiter, "table")
	assertMemberType(cmdLine.delimiter, {"hasEmpty", "hasSpace", "n"}, {"boolean", "boolean", "number"})
	...
	argument = cmdLine:searchByDelimiter("-s", "--start")
	assertEq(argument.n, 1)
